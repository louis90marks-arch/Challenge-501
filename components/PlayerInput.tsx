...
const PlayerInput: React.FC<PlayerInputProps> = ({ onSubmit, isLoading, disabled, clubName }) => {
  ...
  useEffect(() => {
    let interval: number;
    if (isLoading) {
      const messages = ['Verifying stats', 'Searching archives', 'Calculating total', 'Confirming data'];
      let i = 0;
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-2xl mx-auto mb-8">
        ...
        >
          {isLoading ? (
            <>
              ...
              <span className="text-xs uppercase tracking-tight">Checking</span>
            </>
          ) : (
            <span>Submit</span>
          )}
      ...
      <p className="text-center text-slate-500 text-xs mt-3 uppercase tracking-widest font-bold">
        Fast-tracking data verification...
      </p>
    </form>
  );
};
